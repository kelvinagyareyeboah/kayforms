...
function WizardNav({ step, setStep, profileForm, preferencesForm }) {
  const stepConfig = [
    { n: 1, label: "Profile" },
  ...
  const canNavigateTo = (targetStep) => {
    ...
    if (targetStep === 2 && profileForm.valid) return true;
    if (targetStep === 3 && profileForm.valid && preferencesForm.valid) return true;
    return false;
  };
      ...
      {stepConfig.map(({ n, label }, idx) => (
        <React.Fragment key={n}>
          <div
            ...
            onClick={() => canNavigateTo(n) && setStep(n)}
            style={{ cursor: canNavigateTo(n) ? "pointer" : "default" }}
          >
            <div className="step-dot">{step > n ? "✓" : n}</div>
            <span className="step-label">{label}</span>
          </div>
          {idx < stepConfig.length - 1 && <div className="step-divider" />}
        </React.Fragment>
      ))}
    </nav>
  );
      ...
      <div className="inspector-body">
        {forms.map(({ id, form }) => (
          <div className="form-block" key={id}>
            ...
            <pre>
              {JSON.stringify(
                { values: form.values, errors: form.errors, dirty: form.dirty },
                null,
                2
              )}
            </pre>
        ...
        <div className="bench-header">
          <div>
            ...
            <p className="bench-sub">
              50 fields update per frame via granular signals. Only the changed cell 
              re-renders — no virtual DOM diffing, no full tree reconciliation.
            </p>
          </div>
...
export default function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    const preferenceErrors = await preferencesForm.store.validateAll();
    const paymentErrors = await paymentForm.store.validateAll();
